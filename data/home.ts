export const featuredArtworks = [
  {
    title: "Paz Interior",
    collection: "Elegirse",
    technique: "Acrílico sobre lienzo tela",
    image: "/images/obras/paz-interior.webp",
    ratio: "square",
    phrase:
      "No se trata de tenerlo todo en calma, sino de encontrar calma en medio de todo."
  },
  {
    title: "Intuición",
    collection: "Elegirse",
    technique: "Acrílico sobre lienzo en madera",
    image: "/images/obras/intuicion.webp",
    ratio: "portrait",
    phrase: "Cuando el ruido se detiene, la intuición habla."
  },
  {
    title: "Actitud",
    collection: "Elegirse",
    technique: "Acrílico sobre lienzo en madera",
    image: "/images/obras/actitud.webp",
    ratio: "portrait",
    phrase: "No es arrogancia, es la certeza de quien ya se eligió."
  },
  {
    title: "Expectativa",
    collection: "Obra única",
    technique: "Acrílico sobre lienzo en madera",
    image: "/images/obras/espectativa.webp",
    ratio: "portrait",
    phrase: "Una pausa cargada de futuro."
  }
];

export const notes = [
  {
    title: "Sobre elegirse cada día",
    category: "Proceso",
    excerpt:
      "Una reflexión sobre la calma, la intuición y la decisión íntima de volver a una misma."
  },
  {
    title: "El poder de lo simbólico",
    category: "Símbolos",
    excerpt:
      "Cómo una imagen puede guardar una emoción y abrir otra forma de mirar."
  },
  {
    title: "Belleza, verdad y presencia",
    category: "Taller",
    excerpt:
      "Notas sobre crear desde lo sensible sin perder fuerza, oficio ni presencia."
  }
];
